	...
	public void InstantiateObstacle(GameObject[] obstacles)
	{
		...
		if (obstacles != null && obstacles.Length == 3) {
			if (nextObstacle.isSurprise) {
				...
				obstacles[random].GetComponent<Rigidbody2D>().linearVelocity = Vector2.right * -4 * ArcadeBikesManager.Instance.speedMultiplier;
				...
				for (int i = 0; i < nextObstacle.obstacletypes.Length; i++) {
					if (obstacles[i] != null) {
						...
						obstacles[i].GetComponent<Rigidbody2D>().linearVelocity = Vector2.right * -4 * ArcadeBikesManager.Instance.speedMultiplier;
