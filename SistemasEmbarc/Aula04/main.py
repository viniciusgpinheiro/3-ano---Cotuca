...
ledY = machine.Pin(14, machine.Pin.OUT)
ledG = machine.Pin(27, machine.Pin.OUT)
def ligar_semaforo(distancia):
    # Estado 1: Muito perto (Vermelho)
    if distancia < 10:
        print("Pare! Distância:", distancia)
        ledR.value(1)
        ledY.value(0)
        ledG.value(0)
    
    # Estado 2: Atenção (Amarelo)
    elif 10 <= distancia < 50:
        print("Atenção! Distância:", distancia)
        ledR.value(0)
        ledY.value(1)
        ledG.value(0)
    
    # Estado 3: Livre (Verde)
    else:
        print("Livre. Distância:", distancia)
        ledR.value(0)
        ledY.value(0)
        ledG.value(1)
    

WIFI_SSID = "iPhone de Vinicius"
WIFI_PASS = "guepo123"
...
def cbTrataMsg(topic, msg):
    try:
        corpo_mensagem = msg.decode()
        print(f"\n[MSG RECEBIDA] Tópico: {topic.decode()} | Conteúdo: {corpo_mensagem}")
        
        dados = ujson.loads(corpo_mensagem)
        
        if "distancia" in dados:
            valor_distancia = dados["distancia"]
            ligar_semaforo(valor_distancia)
        else:
            print("JSON recebido não contém a chave 'distancia'")
...
while True:
    try:
        ...
        if distancia is not None:
            dados = {
                "id": 999,
                "sensor": round(distancia, 2)
            }
            msg_envio = json.dumps(dados)
            
            print(f"Enviando: {msg_envio}")
